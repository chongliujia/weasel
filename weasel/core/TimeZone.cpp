...
namespace weasel
{
    ...
    namespace detail
    {
        ...
        bool readDataBlock(File& f, struct TimeZone::Data* data, bool v1)
        {
            ...
            std::vector<int> localtime;
            localtimes.reserve(timecnt);

            for (int i = 0; i < timecnt; ++i) {
                int32_t gmtoff   = f.readInt32();
                
                uint8_t isdst    = f.readInt8();
                uint8_t abbrind  = f.readInt8();
        ...
        inline void fillHMS(unsigned seconds, struct DataTime& dt)
        {
        ...
        DataTime BreakTime(int64_t t)
        {
            ...
            Data date(days + Date::kJulianDayOf1970_01_01);
        ...
        const struct DateTime& lt, bool postTransition) const 
{
    ...
    if (prior_second < localtime) {
        if (postTransition) {
            ...
            return &localtimes[prior->trans.localtimeIdx];
        }
    }
